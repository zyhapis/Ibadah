import React from "react";
import { useTheme } from "../../utils/themeMode";

export default function JadwalSholatCard({ sholat }) {
  const { time } = sholat;
  const { darkMode } = useTheme();

  const capitalizeFirstLetter = (string) => {
    return string.charAt(0).toUpperCase() + string.slice(1);
  };

  return (
    <div
      className={`rounded-lg shadow p-4 mb-5 grid grid-cols-3 items-center transition duration-300 transform hover:scale-105 ${
        darkMode
          ? "bg-gray-800 text-gray-300 border-gray-700"
          : "bg-white text-gray-800 border-gray-200"
      }`}
      style={{
        boxShadow: darkMode
          ? "2px 2px 6px rgba(0, 0, 0, 0.4), -2px -2px 6px rgba(255, 255, 255, 0.1)"
          : "2px 2px 4px rgba(0, 0, 0, 0.1), -2px -2px 4px rgba(255, 255, 255, 0.3)",
        transition: "transform 0.3s, box-shadow 0.3s",
      }}
    >
      <div
        className={`col-span-2 ${
          darkMode ? "text-yellow-500" : "text-gray-600"
        }`}
      >
        {time.map((prayerTime, index) => (
          <div key={`${prayerTime.prayer}-${index}`}>
            <span>{capitalizeFirstLetter(prayerTime.prayer)}: </span>
            {prayerTime.time}
          </div>
        ))}
      </div>
      <div className="flex justify-end">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="20"
          height="20"
          fill="currentColor"
          className="ml-3"
          viewBox="0 0 16 16"
        >
          <path d="M8.5 5.5a.5.5 0 0 0-1 0v3.362l-1.429 2.38a.5.5 0 1 0 .858.515l1.5-2.5A.5.5 0 0 0 8.5 9V5.5z" />
          <path d="M6.5 0a.5.5 0 0 0 0 1H7v1.07a7.001 7.001 0 0 0-3.273 12.474l-.602.602a.5.5 0 0 0 .707.708l.746-.746A6.97 6.97 0 0 0 8 16a6.97 6.97 0 0 0 3.422-.892l.746.746a.5.5 0 0 0 .707-.708l-.601-.602A7.001 7.001 0 0 0 9 2.07V1h.5a.5.5 0 0 0 0-1h-3zm1.038 3.018a6.093 6.093 0 0 1 .924 0 6 6 0 1 1-.924 0zM0 3.5c0 .753.333 1.429.86 1.887A8.035 8.035 0 0 1 4.387 1.86 2.5 2.5 0 0 0 0 3.5zM13.5 1c-.753 0-1.429.333-1.887.86a8.035 8.035 0 0 1 3.527 3.527A2.5 2.5 0 0 0 13.5 1z" />
        </svg>
      </div>
    </div>
  );
}
