  ...
  return (
    <div className="w-full">
      ...
      >
        <div
          onClick={() => setIsOpenSelect((prev) => !prev)}
          className={`px-3 flex gap-2 justify-between active:bg-yellow-50 border-slate-300 border w-full items-center h-full ${
            isOpenSelect ? "rounded-t-lg" : "rounded-lg hover:border-yellow-400"
          }`}
        >
          {currentTitle && (
            <>
        ...
        <div
          className={`scrollbar overflow-auto max-h-40 absolute z-30 -mt-[1px] w-full flex-col p-2 gap-1 border text-[#111827] bg-white border-slate-300 rounded-b-lg ${
  ...
  return (
    <span
      ...
      className={`px-3 py-1 inline-flex justify-between items-center rounded-md ${
        currentValue === value
          ? "bg-yellow-500 text-[#111827] font-medium"
          : "hover:bg-yellow-100"
      }`}
    >
      {children}{" "}
